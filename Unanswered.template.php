			...
			<div class="', $post['alternate'] == 0 ? 'windowbg' : 'windowbg2', ' core_posts">
				...
				<div class="content">
					...
					<div class="topic_details">
						...
						<span class="smalltext">&#171;&nbsp;', $txt, ' ', $txt['by'], ' <strong>', $post['poster']['link'], ' </strong> ', $txt['on'], '<em> ', $post['time'], '</em>&nbsp;&#187;</span>
					</div>
...
function template_unread()
{
	...
	$mark_read = array(
		'view_posts' => array('text' => 'unanswered_view_posts', 'lang' => true, 'url' => $scripturl . '?action=unanswered'),
	);

	if ($settings['show_mark_read'])
	{
		// Generate the button strip.
		$mark_read['markread'] = array('text' => !empty($context['no_board_limits']) ? 'mark_as_read' : 'mark_read_short', 'image' => 'markread.gif', 'lang' => true, 'url' => $scripturl . '?action=markasread;sa=' . (!empty($context['no_board_limits']) ? 'all' : 'board' . $context['querystring_board_limits']) . ';' . $context['session_var'] . '=' . $context['session_id']);
	...
	if (!empty($context['posts']))
	{
		...
		if ($showCheckboxes)
			echo '
							<th scope="col" width="22%">
								<a href="', $scripturl, '?action=unread', $context['showing_all_topics'] ? ';all' : '', $context['querystring_board_limits'], ';sort=last_post', $context['sort_by'] == 'last_post' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt, $context['sort_by'] == 'last_post' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a>
							</th>
							<th class="last_th">
		...
		foreach ($context['posts'] as $post)
		{
			...
			$color_class = '';
			if (strpos($post['class'], 'sticky') !== false)
				$color_class = 'stickybg';
			if (strpos($post['class'], 'locked') !== false)
		...
		if (!empty($context['posts']) && !$context['showing_all_topics'])
...
function template_replies()
{
	...
	if (!empty($context['posts']))
	{
		...
		if ($showCheckboxes)
				echo '
							<th scope="col" width="22%">
								<a href="', $scripturl, '?action=unreadreplies', $context['querystring_board_limits'], ';sort=last_post', $context['sort_by'] === 'last_post' && $context['sort_direction'] === 'up' ? ';desc' : '', '">', $txt, $context['sort_by'] === 'last_post' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a>
							</th>
							<th class="last_th">
								<input type="checkbox" onclick="invertAll(this, this.form, \'topics[]\');" class="input_check" />
							</th>';
						...
						<tr>
							...
							<td class="subject ', $color_class2, ' windowbg2">
								<div>
									...
									<a href="', $post['new_href'], '" id="newicon', $post['topic'], '"><img src="', $settings['lang_images_url'], '/new.gif" alt="', $txt['new'], '" /></a>
									<p>
										', $txt['started_by'], ' <strong>', $post['link'], '</strong>
										', $txt['in'], ' <em>', $post['board']['link'], '</em>
										<small id="pages', $post['topic'], '">', $post['pages'], '</small>
									</p>
								</div>
